...
int Fbx_Model_Load(const char *filename, const char *path_name)
{
	...
	for (int i = 0; i < MODEL_MAX; i++)
	{
		...
		HRESULT hr = D3DXLoadMeshFromFBX(filename, D3DXMESH_MANAGED, pDevice, NULL, &g_Model[i].pMaterial, NULL, &g_Model[i].MaterialCount, &g_Model[i].pMesh, 0.1f);	//| D3DXMESH_32BIT
		//エラー処理
		if (FAILED(hr))
		{
