...
void MeshWall_Init(int width, int height, int texId)
{
	...
	{
		vbuf[i].position = D3DXVECTOR3((FLOAT)(i % (g_width + 1)) + -g_width / 2, (FLOAT)(-i / (g_width + 1)) + g_height, 0.0f);
		vbuf[i].normal = D3DXVECTOR3(0.0f, 0.0f, -1.0f);
	...
	// 頂点インデックスの確保
	int index_num = 0;	//縮退ポリゴンの設定が行われた回数
	for (int i = 0; i < g_index_num / 2; i++)
	{
		//縮退ポリゴン
		if ((g_width + 1) + (g_width + 2) * index_num == i && i != 0 && i != g_index_num / 2 && g_height > 1)
		{
			ibuf[i + i] = (unsigned short)(i - 1 - index_num);
			ibuf[i + i + 1] = (unsigned short)((g_width + 1) + i - index_num);
			index_num++;
		}
		//通常インデックス
		else
		{
			ibuf[i + i] = (unsigned short)((g_width + 1) + (i - index_num));
			ibuf[i + i + 1] = (unsigned short)(i - index_num);
		}
	}
