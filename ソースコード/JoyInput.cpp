...
static bool g_old_press = false;
static bool g_now_buttons = false;
...
bool GetButton_isTrigger(int butttonIndex)
{
	//1F前がfalseで現在もfalseであればTrigger
	g_now_buttons = nowInput.rgbButtons[butttonIndex] && !g_old_press;
	//1F前の入力状態記憶
	g_old_press = nowInput.rgbButtons[butttonIndex];

	return g_now_buttons;
}
